"""
This program implements a command-line interface for managing stock information.
"""
import argparse
import os
import pandas as pd
...
class StockManager(Cmd):
    ...
    intro = "\nWelcome to Stock Manager. The following commands are available:\n" \
            ...
            "2. query <column=value>       - Search for stock items by specific criteria\n" \
    ...
    def do_add_data(self, directory):
        """
        Import CSV files from a directory and consolidate them into a single dataset.
    ...
    def do_generate(self, output_file):
        ...
        summary = self.data.groupby('category').agg({
            'quantity': 'sum',
            'unit_price': 'mean'
        }).rename(columns={'quantity': 'Total Quantity', 'unit_price': 'Average Price'})
    ...
    def do_show_top(self, count):
        ...
        try:
            # Si 'count' est un entier, pas besoin d'appliquer 'strip()'.
            if isinstance(count, int):
                count = count
            else:
                # Si 'count' est une chaîne, on applique 'strip()' et on le convertit en entier.
                count = int(count.strip()) if count.strip() else 5
